...
var Chess = function(fen_)
{
	...
	function Evaluate()
	{
		...
		if (eval_mode & 1)
		{
			...
			let ratio = mat0 * 1.0 / (mat0 + mat1) - 0.5;
		...
		if (eval_mode & 2)
		{
			...
			if (mat0 <= 5000)
			{
				let king = kings[WHITE],
					king2 = kings[BLACK];
				score -= (Abs(Filer(king) * 2 - 7) + Abs(Rank(king) * 2 - 7)) * 25;
				score += (Abs(Filer(king) - Filer(king2)) + Abs(Rank(king) - Rank(king2))) * 40;
