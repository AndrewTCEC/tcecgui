...
const {
	_, A, ArrayJS, Attrs, CACHE_IDS, CacheId, Clamp, Class, Clear, Contain, CreateNode, DeepCopy, DefaultArray,
	DefaultFloat, DefaultInt, DefaultObject, E, Format, FormatFloat, FormatPercent, FormatUnit, From, FromSeconds,
	FromTimestamp, HasClass, HasClasses, HexString, Hide, HTML, Id, Index, InsertNodes, IsDigit, IsFloat, IsObject,
	IsString, Keys, Lower, Merge, Now, Pad, Parent, ParseJSON, PI, QueryString, S, Show, Split, Style, TEXT, TextHTML,
	Title, Toggle, Uint8From, Undefined, Upper, Visible, VisibleHeight, VisibleWidth,
} = require('./common.js');

const real_now = Date.now.bind(global.Date);
...
[
	...
	test(`CreateNode:${id}`, () => {
		const node = CreateNode(tag, html, attrs);
...
// DeepCopy
[
	[{x: [1, 2, 3]}, ['x', 1, 99], {x: [1, 99, 3]}, {x: [1, 2, 3]}],
].forEach(([dico, path, answer_dico, answer], id) => {
	test(`DeepCopy:${id}`, () => {
...
[
	...
	test(`HasClass:${id}`, () => {
		const node = CreateNode('div', '', {class: sclass});
...
[
	...
	[15, 6, '', '00000f'],
	[15, 6, '0x', '0x00000f'],
	[255, undefined, undefined, 'ff'],
	[65535, undefined, undefined, 'ffff'],
	[-1, undefined, undefined, ''],
].forEach(([number, padding, prefix, answer], id) => {
	test(`HexString:${id}`, () => {
		expect(HexString(number, padding, prefix)).toEqual(answer);
	});
});
...
[
	...
	test(`HTML:${id}`, () => {
		const soup = CreateNode('div', shtml);
