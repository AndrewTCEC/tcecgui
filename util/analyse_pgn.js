...
const engine_check = 'LCZero 0.27.0d-Tilps-dje-magic_JH.94-100',
	OPTIONS = {},
	REPLACES = {
		bonus: 'Bonus',
		cup: 'Cup ',
...
{
	data.split(/\r?\n\r?\n(?=\[)/).forEach((split, id) => {
		const dico = getPgnStats(split, `${origin}:${id}`);
		Keys(dico).forEach(key => {
			const entry = DefaultArray(result, key, []),
				value = dico[key];
			entry.push(value);
		});
	});
...
{
	const dico = parsePgn('', data, 7, origin);
	if (!dico || !dico.Headers || !dico.Moves)
		return {};

	// s18, cup5, ...
	const left = origin.split(' / ')[0],
	...
	average[0] = sum_times[0]? Floor(sum_moves[0] / sum_times[0]) : 0;
	average[1] = sum_times[1]? Floor(sum_moves[1] / sum_times[1]) : 0;

	all_values.forEach((values, id) => {
		const color = ['White', 'Black'][id],
			half = sum_moves[id] / 2,
			name = headers[color],
		...
		{
			const keys = Keys(options).filter(key => !skipped_keys[key]);
			if (keys.length)
			{
				LS([
					name,
		...
		// add synonym
		const name2 = [name, event, threads, gpus].map(item => item || '').join('|');
		if (threads)
...
{
	const headers = 'Engine|Speed|nps|Event|Threads|GPUs|Games'.split('|'),
	...
	names.forEach(key => {
		...
		{
			const name_key = splits.slice(0, 2).join('|'),
	...
	// 3) sort results
	const sort_alpha = OPTIONS.alpha,
	...
	const keys = Keys(result).filter(key => {
	...
	keys.sort((a, b) => {
		const sa = a.split('|'),
			sb = b.split('|');
		if (sort_engine && sa[0] != sb[0]) return sa[0].localeCompare(sb[0]);
		if (sort_event && sa[1] != sb[1]) return sb[1].localeCompare(sa[1]);
		if (sort_alpha && sa[0] != sb[0]) return sa[0].localeCompare(sb[0]);
		return result[b][0] - result[a][0];
	});

	// 4) output
	let prev_items;
	const header = headers.map((item, id) => (item + spaces[id]).slice(0, maxs[id])),
	...
	if (worsts.length)
	{
		...
		{
			const vector = worst.map(item => (item + '').padStart(8));
			LS(vector.join(' '));
		}
	}
	return text;
